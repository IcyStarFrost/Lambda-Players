...
function TOOL:LeftClick( tr )
    ...
    if ( SERVER ) then ent:TypeMessage( ent:GetTextLine( self:GetClientInfo( "texttype" ) ) ) end
    return true
end
...
function TOOL:RightClick( tr )
    ...
    if ( SERVER ) then ent:TypeMessage( self:GetClientInfo( "textline" ) ) end
    return true
end

function TOOL.BuildCPanel( pnl )
    local box = pnl:ComboBox( "Text Type", "lambdatexttester_texttype" )
    for textType, _ in pairs( LambdaTextTable ) do box:AddChoice( textType, textType ) end
