...
local function OnClick( tool, ent, info )
    if !IsValid( ent ) or !ent.IsLambdaPlayer then return false end

    if ( SERVER ) then
        ent:PlaySoundFile( tool:GetClientInfo( info ) )
    end
...
function TOOL:LeftClick( tr )
    return OnClick( self, tr.Entity, "voicetype" )
end

function TOOL:RightClick( tr )
    return OnClick( self, tr.Entity, "voicelinepath" )
end

function TOOL.BuildCPanel( pnl )
    local box = pnl:ComboBox( "Voice Type", "lambdavoicelinetester_voicetype" )
    for k, _ in pairs( LambdaVoiceLinesTable ) do box:AddChoice( k, k ) end
